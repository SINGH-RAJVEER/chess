...
import type { Color, PieceType } from "../../db/schema";
  ...
  .handler(async ({ data: { from, to, gameId } }) => {
    try {
      ...
      if (moveDetails.captured) {
          ...
          await db.delete(schema.pieces).where(and(eq(schema.pieces.gameId, currentGame.id), eq(schema.pieces.square, targetSquare)));
      }
...
export async function applyMove(gameId: number, from: number, to: number) {
  ...
  if (moveDetails.captured) {
      ...
      await db.delete(schema.pieces).where(and(eq(schema.pieces.gameId, gameId), eq(schema.pieces.square, targetSquare)));
  }
  ...
  const finalMove = await db.query.moves.findFirst({ where: eq(schema.moves.gameId, gameId), orderBy: desc(schema.moves.moveNumber) });
