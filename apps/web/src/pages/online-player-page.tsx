import type { BoardResponse, Color, PieceType, QueueStatusResponse } from "@chess/types";
...
export default function OnlinePlayerPage() {
	...
	useEffect(() => {
		if (!playerId || (boardData && boardData.id !== 0)) {
			setQueueStatus((current) => (current.status === "matched" ? current : { status: "idle" }));
			return;
		}
	...
	const rotation = userColor === "Black" ? 180 : 0;
	...
	const handleSquareClick = async (squareIndex: number) => {
		if (!boardData || boardData.id === 0) return;
	...
	return (
		<div className="flex min-h-screen flex-col bg-zinc-950 font-sans text-zinc-300">
			<Header
				...
				isRestarting={isResetPending || isJoiningQueue}
