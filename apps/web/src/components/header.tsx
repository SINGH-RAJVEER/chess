		...
		<header class="bg-stone-900 text-white shadow-md px-6 py-3 sticky top-0 z-50 flex items-center justify-between">
			...
			<div class="flex items-center gap-3 justify-end w-1/4">
					...
					<div class="relative">
						...
						>
							...
							<svg
								...
								viewBox="0 0 24 24"
							>
								<title>Toggle time control dropdown</title>
								<path
						...
						<Show when={isOpen()}>
							<button
								...
								class="fixed inset-0 z-10 cursor-default w-full h-full border-none p-0 bg-transparent"
				...
				<button
					...
					class={`font-bold py-2 px-4 rounded-lg transition-all uppercase tracking-wider text-xs border border-stone-200 shadow-lg disabled:opacity-50 hover:scale-105 active:scale-95 flex items-center gap-2 ${
						props.queueStatus === "queued"
							? "bg-indigo-600 text-white cursor-wait animate-pulse"
							: "bg-stone-100 hover:bg-white text-stone-900"
					}`}
				>
					<Show when={props.queueStatus === "queued"}>
